...
public class BpSimDataRootExport implements BpSimXmlConstants {

    /** default namespaces for definitions */
    protected static final Set<String> defaultNamespaces = new HashSet<>(Arrays.asList(XSI_PREFIX, FLOWABLE_EXTENSIONS_PREFIX));

    public static void writeRootElement(BpSimModel model, XMLStreamWriter xtw, String encoding) throws Exception {
        xtw.writeStartDocument(encoding, "1.0");

        // start definitions root element
        xtw.writeStartElement(ELEMENT_BPSIM_DATA);
        xtw.setDefaultNamespace(BPSIM_NAMESPACE);
        xtw.writeDefaultNamespace(BPSIM_NAMESPACE);
